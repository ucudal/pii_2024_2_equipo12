...
namespace Ucu.Poo.DiscordBot.Commands
{
    ...
    public class SelectPokemonCommand : ModuleBase<SocketCommandContext>
    {
        ...
        [Command("select")]
        [Summary(
            "Permite al usuario seleccionar hasta 6 Pokémon del catálogo por sus índices. Uso: !selectpokemon <1 2 ... 6>")]
        public async Task ExecuteAsync(
            [Remainder] [Summary("Índices de los Pokémon a seleccionar separados por espacios")]
            string indices)
        {
            string displayName = CommandHelper.GetDisplayName(Context);
            var battlesList = Facade.Instance.GetBattlesList();
            ...
            var verificationResult = Facade.Instance.InitialVerifications(player, opponent, battle, false);
            if (verificationResult != null)
            {
                await ReplyAsync(verificationResult.Value.message);
                return;
            }
